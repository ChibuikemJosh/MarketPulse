...
TIME_FORMAT = '%Y-%m-%d %H:%M:%S'  # Standard dateti
        ...
        for symbol, name in names_snapshot.items():
            if symbol not in existing_data:
                ...
                if name not in existing_data[symbol]:
                    existing_data[symbol].insert(0, name)  # Insert as first alias
...
def calc_price_change(symbol):
    """Calculate percentage price change from previous market close.
    Used for trending scores to boost symbols with positive momentum.
    ...
    try:
        ...
        info = ticker.fast_info or ticker.info  # Use fast_info if available for better performance
        current_price = info.get('last_price') or info.get('lastPrice')
        previous_closing_price = info.get('previous_close') or info.get('previousClose')

        # Calculate: (current - previous) / previous * 100
        if previous_closing_price and current_price and previous_closing_price > 0:
            change_pct = ((current_price - previous_closing_price) / previous_closing_price) * 100
            return round(change_pct, 2)
        else:
            print(f"DEBUG: Missing price data for {symbol} (Current: {current_price}, Prev: {previous_closing_price})", flush=True)
...
def update_trends():
    ...
    symbols_to_track = list(BRAND_MAP.keys())  # All symbols in our brand map
    formatted_symbols = [s.replace('.LG', '.NGE') for s in symbols_to_track]
    print(f"\n[!!!] BATCH UPDATE STARTING for {len(symbols_to_track)} symbols...", flush=True)
    ...
    for original_symbol in BRAND_MAP.keys():
        info = MARKET_MAP[original_symbol]
        if info:
            m = info['market']
    ...
    print(f"\n[!!!] TRADINGVIEW BATCH UPDATE STARTING...", flush=True)
            ...
            try:
                ...
                for s in symbols:
                    ...
                    if not final_name and can_call_alpha_vantage_api():
                        print(f"TV Name failed for {s}, trying Alpha Vantage...")
                    ...
                    # Save the winner to your cache
                    with cache_lock:
                        CACHED_NAMES[s] = final_name

                print(f"DEBUG: Successfully updated {len(symbols)} symbols for {market_name}", flush=True)
    ...
    with cache_lock:
        ...
        print(f"DEBUG: Updated trending scores for {len(TRENDING_SCORES)} symbols.", flush=True)
        print(f"Sample Scores: AAPL: {TRENDING_SCORES.get('AAPL')}% | TSLA: {TRENDING_SCORES.get('TSLA')}%", flush=True)
        save_cache_to_disk()
...
def login():
    ...
    """
    session.clear()  # Clear any existing sessions (logout first)
...
def search_suggest():
    ...
    if not user_id:
        try:
            # Anonymous user: weights provided by client
            user_weights = data.get('weights', {})  # Expecting {symbol: weight} from client
        except Exception:
            user_weights = {}
    else:
...
def record_click_endpoint():
    ...
    try:
        with closing(sqlite3.connect('market_data.db')) as conn:
            cursor = conn.cursor()
            # Upsert: Insert new row or increment weight if symbol already exists
            cursor.execute('''
                INSERT INTO global_weights (symbol, weight)
                VALUES (?, 1)
                ON CONFLICT(symbol) DO UPDATE SET weight = weight + 1
            ''', (symbol,))
            conn.commit()
...
def process_trends(df):
    """Analyze trends in stock price data and return trend segments.
    This is a placeholder for existing logic.
    Should call your polyfit/segmentation function here.

    Args:
        df: pandas DataFrame with price history

    Returns:
        list: Trend segments with start/end indices and mood classification
    """
    # Example hardcoded segments based on previous logic:
    # In real app, call your polyfit/segmentation function here.
    segments = [
        {"start": 0, "end": 10, "slope": -1.5, "intercept": 275.0, "mood": "Bearish"},
        {"start": 10, "end": 25, "slope": 1.2, "intercept": 255.0, "mood": "Bullish"}
    ]
    return segments
...
def index():
    ...
    trending_stocks = []
    with cache_lock:
        sorted_movers = sorted(TRENDING_SCORES.items(), key=lambda x: abs(x[1]), reverse=True)[:10]
