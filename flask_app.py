...
USER_WEIGHT_CACHE = {}  # Cache of user-specific click weights indexed by user_id {user_id: {symbol: weight}}
...
def get_user_weights(user_id):
    ...
    with cache_lock:
        if user_id in USER_WEIGHT_CACHE:
            return USER_WEIGHT_CACHE[user_id]
    ...
    with cache_lock:
        USER_WEIGHT_CACHE[user_id] = user_weights
...
def record_click(symbol, user_id=None):
    ...
    with cache_lock:
        if user_id:
            if user_id not in USER_WEIGHT_CACHE:
                USER_WEIGHT_CACHE[user_id] = {}

            USER_WEIGHT_CACHE[user_id][symbol] = USER_WEIGHT_CACHE[user_id].get(symbol, 0) + 1.0
